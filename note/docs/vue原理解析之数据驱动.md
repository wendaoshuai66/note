...
1.首先缓存了原型上的 $mount 方法，然后对  $mount 重新定义
...
3.很关键的逻辑 如果没有定义 render 方法，则会把 el 或者 template 字符串转换成 render 方法。
...
通过上述代码：mountComponent 核心就是先实例化一个渲染Watcher，
...
1.我们在平时的开发工作中手写 render 方法的场景比较少

2.的比较多的是 template 模板，在之前的 mounted 方法的实现中，会把 template 编译成 render 方法，
...
vm.$createElement 方法定义 ---->是在执行 initRender 方法的时候.
...
Virtual DOM 这个概念应该不会陌生了吧，它产生的前提是浏览器中的DOM是很昂贵的,为了更直观的感受，我们可以简单的把一个简单的 div 元素的属性都打印出来，如图所示：

![](https://wendaoshuai66.github.io/study/note/images/vue4.png)

可以看到，真正的 DOM 元素是非常庞大的，因为浏览器的标准就把 DOM 设计的非常复杂。当我们频繁的去做 DOM 更新，会产生一定的性能问题。

但是Virtual DOM就是利用原生的js对象去描述一个DOM节点，-----》它会比创建一个DOM代价小很多。
...
Vue.js 中的 Virtual DOM 的定义还是略微复杂一些的，因为它这里包含了很多 Vue.js 的特性。这里千万不要被这些茫茫多的属性吓到，实际上 Vue.js 中 Virtual DOM 是借鉴了一个开源库 [snabbdom ](https://github.com/snabbdom/snabbdom)的实现，然后加入了一些 Vue.js 特色的东西。所以如果想深入了解 Vue.js 的 Virtual DOM 前不妨先阅读这个库的源码，因为它更加简单和纯粹。

###总结

VNode----》就是对真实DOM的一种抽象描述
...
#####为什么Virtual DOM是非常轻量和简单的。

因为VNODE 是映射到真实的DOM的渲染的，不需要包含操作DOM的方法。


Virtual DOM除了定义数据结构，映射到真实的DOM需要经历 -----》VNode 的 create、diff、patch 等过程
...
2.另一个场景是当编译 slot、v-for 的时候会产生嵌套数组的情况，会调用 normalizeArrayChildren 方法，接下来看一下它的实现：
...
1.遍历 children，获得单个节点 c，

2.然后对 c 的类型判断，如果是一个数组类型，---->则递归调用 normalizeArrayChildren; 如果是基础类型，----->则通过 createTextVNode 方法转换成 VNode 类型；否则就已经是 VNode 类型了，如果 children 是一个列表并且列表还存在嵌套的情况，则根据 nestedIndex 去更新它的 key。这里需要注意一点，在遍历的过程中，对这 3 种情况都做了如下处理：如果存在两个连续的 text 节点，会把它们合并成一个 text 节点。
...
createChildren的逻辑很简单，实际上是遍历子虚拟createElm优先级，递归调用，这是一种常用的深度优先的遍历算法，这里要注意的一点是在遍历过程中会把vnode.elm作为父容器的DOM字节占位符传入。

接着再调用invokeCreateHooks方法执行所有的create的钩子并把vnodepush到insertedVnodeQueue中。
...
最后调用insert方法把DOM插入到父节点中，因为是递归初始化，子元素会优先调用insert，所以整个vnode树例程的插入顺序是先子后父。来看一下insert方法，它的定义在src/core/vdom/patch.js上。
...
insert逻辑很简单，调用一些nodeOps把子例程插入到父节点中，这些辅助方法定义在src/platforms/web/runtime/node-ops.js中：
...
其实就是调用原生DOM的API进行DOM操作，看到这里，很多同学恍然大悟，原来Vue是这样动态创建的DOM。

在createElm过程中，如果vnode例程不包含tag，则它有可能是一个注释或者纯文本例程，可以直接插入到父元素中。在我们这个示例中，最内层就是一个文本vnode，它的text值取的就是之前的this.message的值Hello Vue!。

再回到patch方法，首次渲染我们调用了createElm方法，这里称为的parentElm是oldVnode.elm的父元素，在我们的例子是id为#appdiv的父元素，也就是Body；实际上整个过程就是递归创建了一个完整的DOM树并插入到Body上。

最后，我们根据之前递归createElm生成的vnode插入顺序数组，执行相关的insert钩子函数，这部分内容我们之后会详细介绍。


##总结

那么至此从主线上把模板和数据如何渲染成最终的DOM的过程分析完成了，我们可以通过下图更直观地看到从初始化Vue到最终渲染的整个过程。

![](https://wendaoshuai66.github.io/study/note/images/new-vue.png)

我们这里只是分析了最简单和最基础的场景，在实际项目中，我们是把页面拆成很多组件的，Vue另一个核心思想就是组件化。那么下一章我们就来分析Vue的组件化过程。
